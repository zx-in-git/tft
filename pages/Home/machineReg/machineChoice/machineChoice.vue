		...
		<radio-group @change="getTermNum">
			<view class="view" v-if="termList.length != 0">
				<label class="term-info" v-for="(item, index) in termList" :key="index">
					<view class="checkbox-view checkbox-text-one">
						<text class="checkbox-text">SN编号：{{ item.sn }}</text>
					</view>
					<view class="checkbox-view-radio">
						<radio class="checkbox" color="#f98021" v-model="item.sn" />
					</view>
...
export default {
	...
	methods: {
		// 获取终端信息
		getTermList(){
			net({ url:"/V1/getNoBindMachines", method:'get',
	            success: (res) => {
