<template>
	<view class="background">
		...
		<view class="body">
			...
			<view class="bview">
				<text>可提现金额：</text>
				<text class="mainColor">{{balance }}</text>
				<text>元</text>
			</view>
			<view class="hengxian"></view>
			<view class="bview d-flex">
				<input class="qbtxje" type="number" placeholder="请输入提现金额" @input="moneyInput" v-model="money" />
				<view @click="cashAll" class="allDraw">全部提现</view>
			</view>
			<view class="hengxian"></view>
			<view class="bview">
				<text>应到金额：</text>
				<text class="mainColor">{{ reachBalance }}</text>
...
export default {
	...
	data() {
		return {
			...
			resultInfo: { result: '请选择提现钱包', value: '' },
			// 用户信息
			UserInfo: [],
			// 提现设置信息
			cashsetUp: [],
	...
	      	})
		...
		// 获取提现设置信息
		getPoint(val){
			net({ url:"/V1/getPoint", method:'get', data:{type:val},
	            success: (res) => {
					this.loadModal.show = false;
					if (res.data.success) {
						this.cashsetUp = res.data.success.data;
					} else {
						uni.showToast({ title: res.data.error.message, icon: 1500 });
					}
	            }
	      	})
		},
		...
		// 获取默认结算卡信息
		getDefaultCard(){
			net({ url:"/V1/getBankDefault", method:'get',
	            success: (res) => {
					this.loadModal.show = false;
	...
	      	})
		...
		moneyInput(event){
			...
			this.reachBalance = (settleMoney - this.cashsetUp.rate_m).toFixed(2);
		},
